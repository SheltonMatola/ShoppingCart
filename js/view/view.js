...
export class CartView {
  ...
  open() { if (this.refs.panel) { this.refs.panel.classList.add('open'); this.refs.panel.setAttribute('aria-hidden', 'false'); } }
  close() { if (this.refs.panel) { this.refs.panel.classList.remove('open'); this.refs.panel.setAttribute('aria-hidden', 'true'); } }
  toggle() { if (this.refs.panel) { this.refs.panel.classList.contains('open') ? this.close() : this.open(); } }
  ...
  bindToggle(handler) {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (this.refs.panel.classList.contains('open')) {
        if (!this.refs.panel.contains(target) && !this.refs.bag.contains(target)) this.close();
      }
    });
  }
